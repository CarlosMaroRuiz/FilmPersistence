...
# Configuración
VIDEO_FOLDER = 'static/videos'  # Cambiado a static/videos para mejor organización
...
def update_progress():
    ...
    existing = conn.execute(
        'SELECT id FROM watch_history WHERE user_id = ? AND video_name = ?',
        (session['user_id'], video_name)
    ).fetchone()
    
    if existing:
        # Actualizar registro existente
        conn.execute(
            'UPDATE watch_history SET progress = ?, watched_at = CURRENT_TIMESTAMP WHERE id = ?',
            (progress, existing['id'])
        )
    else:
        # Crear nuevo registro
        conn.execute(
            'INSERT INTO watch_history (user_id, video_name, progress) VALUES (?, ?, ?)',
            (session['user_id'], video_name, progress)
        )
